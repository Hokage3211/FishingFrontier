...
execute as @e[type=minecraft:fishing_bobber] if score @s fisherman_id = $temp fr_data run tag @s add matched_bobber
execute store result score $result fr_data run tp @s @n[type=minecraft:fishing_bobber,tag=matched_bobber]
tag @e[type=minecraft:fishing_bobber,tag=matched_bobber] remove matched_bobber
#debug line to get result
#tellraw @a ["", {"selector":"@s"}, ": ", {"score":{"objective":"fr_data", "name":"$result"}}]

#if failed to teleport, finalize marker step
execute if score $result fr_data matches 0 run function fishingrework:fish_loot_system/tracker_final_step
